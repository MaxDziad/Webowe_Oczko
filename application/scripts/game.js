...
function RenderBoard(){
    for(let i = 0; i < listOfPlayers.length; i++){
        ...
        <div class="card" data-player="${listOfPlayers[i].username}">
            <p class='card_points'>
                <div class="image"><img src="" id="symbol"></div>
            </p>
        </div>
        `;
    }
...
function PutCard(player, card) {
    ...
    console.log(cardPlace.querySelector('#symbol'));
    let cardImage = cardPlace.querySelector('.image img');
    cardImage.src = card.GetImageSymbolPath();
    console.log(cardImage.src);
    player.drawnCards += 1;
    player.currentPoints += CalculateCardPoints(player, card);
    cardPlace.innerHTML = card.value;
